...
function selectIndex(object){
  ...
  if (typeof config.index === 'string') return config.index;

  for (var index in config.index ){
...
function globalFilter(){
  ...
  for (var index in config.index){
    var _val = config.index[index].filter;
    ...
    filter = ejs.TermFilter (key, value);
  ...
  /**
   * FIXME: only one filter spupported
   */
...
var search = {
  ...
  suggest : function (query, filter, type, fields, callback){
    ...
    var _global_filter = globalFilter();
    if (_global_filter){
      _f.must.push(_global_filter);
    }
  ...
  getAggregate : function(key, query, filter, callback){
    ...
    if (query){
      searchBody.query(ejs.QueryStringQuery(query).defaultOperator('AND'))
    }

    // FIXME: Use migrate to bodybuilder from elastic.js;
    var queryObject = searchBody.toJSON();
    queryObject.aggs.results.aggs.request_aggregation.terms.size = 1000;
    queryObject.aggs.results.aggs.request_aggregation_raw.terms.size = 1000;
    client.search({
      index: selectIndex(),
      type: 'image',
      size: 0,
  ...
  getRandomImagesFromArchiveByID : function (archive_id, callback){
    ...
    var _global_filter = globalFilter();
    if (_global_filter){
      searchBody.filter(ejs.BoolFilter().must([_global_filter]));
    }
  ...
  queryStringSearch : function (query, filter, offset, limit, callback){
    ...
    if(filter){
      ...
      requestBody.filter(ejs.BoolFilter().must(_f.must));
    }
